...
class Field:
    ...
    def __init__(self, N: int, C: int, S:List[List[int]]):
        self.C = C
        self.S = S
    ...
    def query(self, y: int, x: int, power: int) -> Response:
        ...
        res = Response(self.excavation(y, x, power))
        if res in (Response.BROKEN, Response.FINISH):
            self.is_broken[y][x] = True
        return res
    
    def excavation(self, y: int, x: int, power:int) -> int:
        ex_res = 0
        if S[y][x] > power:
...
class Solver:
    ...
    def __init__(self, N: int, source_pos: List[Pos], house_pos: List[Pos], C: int, S:List[List[int]]):
        self.N = N
...
def main():
    ...
    solver = Solver(N, source_pos, house_pos, C, S)
    solver.solve()
